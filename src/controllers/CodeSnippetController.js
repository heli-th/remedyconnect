...
const Widget = async (req, res) => {
  ...
  if (!key || !table || !view) {
    return res
      .status(400)
      .send("Missing required parameters: base, table, or view");
  }
      ...
      if (response.data.records && response.data.records.length > 0) {
        ...
        const goBackBtn =
          displayType === "dropdown"
            ? ""
            : `<button id="go-back-btn" class="go-back-btn">&larr; Go Back</button>`;
        ...
        if (safe(record["Publisher"]) === "Self Care Decisions") {
          ...
          safe(record["Publisher"]) === "American Academy of Pediatrics (AAP)"
        ) {
          ...
          safe(record["Publisher"]) === "Remedy Medical Contributors" ||
          safe(record["Publisher"]) === "Practice"
        ) {
          html = `
                     <div class="article-detail-full">
                     ${goBackBtn}
          ...
          html = `
                    ...
                    <main>
                        ...
                        <section>
                            <a class="btn" href=" ${safe(
                              record["Article Link"]
                            )}" target="_blank">Download Or View</a>
                        </section>
  ...
  try {
    ...
    let searchHtml = `
            <div class="search-box">
                <input type="text" placeholder="Search articles..." aria-label="Search articles" value="${search.replace(
                  /"/g,
                  "&quot;"
                )}" onkeydown="searchRecords(event)">
            </div>
        `;
                        ...
                        ${paginatedRecords
                          .map((record) => {
                            ...
                            if (displayInGrid.includes("Title")) {
                              const title =
                                record.fields["Article Title"] || "Untitled";

                              cardHtml += `<div class="card-title">${title}</div>`;
                            }
                            if (
                              displayInGrid.includes("Summary") &&
                              record.fields["Article Summary"]
                            ) {
                    ...
                    <ul class="responsive-list">
                        ${paginatedRecords
                          .map((record) => {
                            ...
                            if (
                              displayInList.includes("Summary") &&
                              record.fields["Article Summary"]
                            ) {
