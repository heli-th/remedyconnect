(async () => {
  ...
  if (
    container &&
    (container.getAttribute("data-display") === "list" ||
      container.getAttribute("data-display") === "grid")
  ) {
    ...
    if (!key || !table || !view) {
      container.innerHTML =
        "Missing data attributes: data-key, data-table, or data-view.";
      return;
    }
    ...
    let serverUrl =
      window.NODE_SERVER_URL || "https://externalcontent.remedyconnect.com";

    // Build the URL using window.location
    let url = `${serverUrl}/api/code/code-widget?isclientbase=${encodeURIComponent(
      isclientbase
    )}&key=${encodeURIComponent(key)}&table=${encodeURIComponent(
      table
    )}&view=${encodeURIComponent(view)}&displayType=${encodeURIComponent(
      displayType
    )}`;
    ...
    async function showArticleDetail(slug, opts = { pushState: true }) {
      ...
      try {
        ...
        let detailUrl = `${serverUrl}/api/code/code-widget?isclientbase=${encodeURIComponent(
          isclientbase
        )}&key=${encodeURIComponent(key)}&table=${encodeURIComponent(
          table
        )}&view=${encodeURIComponent(view)}&displayType=${encodeURIComponent(
        ...
        if (
          html.includes('id="go-back-btn"') ||
          html.includes('class="go-back-btn"')
        ) {
          container.innerHTML =
            html +
    ...
    function showDetailFromHtml(slug) {
      ...
      doc.querySelectorAll(".widget-link").forEach((link) => {
        if (
          (link.getAttribute("data-slug") ||
            link.getAttribute("href").split("/").pop()) === slug
        ) {
          found = link;
        }
      });
      ...
      const title =
        found.querySelector(".card-title")?.textContent || "Untitled";
    ...
    container &&
    container.getAttribute("data-display") === "dropdown"
  ) {
    // If it's a dropdown widget, we handle it separately
    // --- BEGIN MODIFIED DROPDOWN LOGIC ---
    document
      .querySelectorAll(".code-snippet-widget")
      .forEach(async (container) => {
        ...
        const displayType =
          container.getAttribute("data-display") || "dropdown";
        const detailPageBaseUrl = container.getAttribute(
          "data-detailPageBaseUrl"
        );
...
Replace "YOUR-DETAIL-PAGE-URL" with the actual path to your detail page.</pre>`;
        ...
        let serverUrl =
          window.NODE_SERVER_URL ||
          "https://externalcontent.remedyconnect.com/"; // Replace with your server URL
        let url = `${serverUrl}/api/code/code-widget?isclientbase=${encodeURIComponent(
          isclientbase
        )}&key=${encodeURIComponent(key)}&table=${encodeURIComponent(
          table
        ...
        async function showArticleDetail(slug) {
          ...
          try {
            let detailUrl = `${serverUrl}/api/code/code-widget?isclientbase=${encodeURIComponent(
              ...
              view
            )}&displayType=${encodeURIComponent(
              displayType
            )}&slug=${encodeURIComponent(slug)}`;
            const res = await fetch(detailUrl);
            const html = await res.text();
